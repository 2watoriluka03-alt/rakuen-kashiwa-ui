...
with tabs[3]:
    ...
    selected_unit = st.selectbox("台番を選択", sorted(all_units["台番"].unique().tolist()))
    hist = all_units[all_units["台番"] == selected_unit].sort_values("日付")
    ...
    st.subheader("特定日カルテ")
    special_days = hist[hist["日"].isin([7, 11, 17, 22, 27])].copy()
...
with tabs[4]:
    ...
    machine = st.selectbox(
        ...
        key="machine_all_data"
    )

    view = all_units[all_units["機種"] == machine].copy()

    day_text = st.text_input(
        "特定日だけ表示（例: 7,11,22）",
        "",
        key="machine_special_days"
    )
    ...
    if days:
        view = view[view["日"].isin(days)]

    sort_col = st.selectbox(
        "並び替え",
        ["日付", "台番", "差枚", "G数", "出率", "末尾"],
        key="machine_sort_col"
    )
    ...
    sort_order = st.radio(
        ...
        ["降順", "昇順"],
        horizontal=True,
        key="machine_sort_order"
    )

    view = view.sort_values(sort_col, ascending=(sort_order == "昇順"))

    st.caption("列名クリックでも並び替えできます。")

    st.dataframe(
        view[["日付", "曜日", "機種", "台番", "末尾", "差枚", "G数", "出率", "下二桁", "下二桁ゾロ目", "完全ゾロ目"]],
        use_container_width=True,
        hide_index=True,
        height=700,
        column_config={
